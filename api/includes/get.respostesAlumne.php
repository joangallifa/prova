<?php
    ...
    try {
        foreach ($jsonA as $dada) {
            ...
            // $sql = $conn->prepare("RgpdDonaRespostesUsuari :QuiContesta, :login, :anyAcademic");
            $sql = $conn->prepare("RgpdCalculaRespostesUsuari :login, :anyAcademic");
            ...
            if (count($outA)>0){
                $jsonO = array('status' => '1','message' => 'S\'ha trobat '.count($outA).' resposta per a alumne/fill');
                $jsonO['results']=$outA;
            }else{
