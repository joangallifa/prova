<?php
 ...
 //    echo "$json";
	...
	try {
		...
		$sql = $conn->prepare("UPDATE RgpdOpcions SET text = :text, descripcio = :descripcio, receptor = :receptor output inserted.guidOpcion WHERE guidOpcion = :guidOpcion");
		$sql->execute(array('text'=>$jsonA['text'],'descripcio'=>$jsonA['descripcio'], 'guidOpcion'=>$jsonA['guidOpcion'], 'receptor'=>$jsonA['receptor']));	
